...
- `concurrency` - 请求队列的最大并发数
- `queue_status_log_interval_ms` - 周期打印当前并发/排队状态到容器日志（毫秒，`<=0` 关闭，默认 60000）
- `queue_timeout` - 队列等待超时时间（毫秒）
- `retry_delay` - 429 初始退避时间（毫秒）
- `max_retry_delay` - 429 退避时间上限（毫秒）
...
QUEUE_STATUS_LOG_INTERVAL_MS=60000
DIRECT_429_COOLDOWN_MS=10000
PROXY_HEALTH_CHECK_INTERVAL_MS=30000
...
**🛡️ 三层身份保护**
- ✨ 扩展身份探针检测：关键词匹配（问模型/平台/系统提示词等），仅记录并透传真实上游响应
